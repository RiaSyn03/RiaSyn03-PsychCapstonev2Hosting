...
=======
    <div class="row">
        ...
        <table class="table table-striped table-responsive mx-3">
        <thead>
            <div class="panel-body">
                ...
                <th scope="col"><center>Year Level</center></th>
                <th scope="col"><center>Email</center></th>
                <th scope="col"><center>Roles</center></th>
                <th scope="col"><center>Actions</center></th>W
                </tr>
        </thead>
        <tbody id="dynamic-row">
            @foreach($users as $user)
...
=======
                    tableRow = '<tr><td>'+value.idnum+'</td><td>'+value.lname+'</td><td>'+value.fname+'</td><td>'+value.course+'</td><td>'+value.year+'</td><td>'+value.email+'</td><td>'+value.roles+'</td><td><a href="{{ route('admin.users.edit', '+value.id+') }}" class="float-left"><button type="button" class="btn btn-primary btn-sm"><i class="fa fa-edit"></i></button></a><a href="{{ route('admin.impersonate', '+value.id+') }}" class="float-left"><button type="button" class="btn btn-success btn-sm"><i class="fa fa-eye"></i></button></a><form action="{{ route('admin.users.destroy', '+value.id+') }}" method="POST" class="float-left">{{ method_field('DELETE') }}@csrf<button type="submit" class="btn btn-danger btn-sm"><i class="fa fa-trash-o"></i></button></form></td></tr>';
>>>>>>> 106ca1a483bdf725dccae9f53e85da85d3cea71b
